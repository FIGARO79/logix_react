...
# --- INICIO: Forzar HTTPS en el código ---

@app.middleware("http")
async def force_https_middleware(request: Request, call_next):
    # El SchemeMiddleware ya ha procesado la solicitud
    # y ha establecido el 'scheme' correcto basado en 'x-forwarded-proto'.
    
    if request.scope['scheme'] == 'http':
        # Si la solicitud es 'http', construye la URL 'https'
        # reemplazando el scheme y redirige.
        https_url = str(request.url).replace("http://", "https://", 1)
        
        # Devolvemos un 301 (Redirección Permanente)
        return RedirectResponse(https_url, status_code=status.HTTP_301_MOVED_PERMANENTLY)

    # Si ya es 'https', simplemente continúa.
    response = await call_next(request)
    return response

# --- FIN: Forzar HTTPS en el código ---
