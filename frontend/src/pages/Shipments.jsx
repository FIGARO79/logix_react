        ...
        <div className="max-w-[1400px] mx-auto px-6 py-6 font-sans bg-[#fcfcfc] min-h-screen text-zinc-800">
            ...
            <div className="mb-8 border-b border-zinc-200 pb-6 flex justify-between items-end">
                ...
                <div className="flex items-center gap-6">
                    <button
                        ...
                        className="text-[9px] font-medium  uppercase tracking-widest text-zinc-400 hover:text-zinc-900 transition-colors"
                    >
                        Sincronizar
                    </button>
                    <div className="text-[9px] font-medium  text-zinc-400 uppercase tracking-widest border-l border-zinc-200 pl-6">
            ...
            {loading ? (
                ...
                <div className="text-center py-20 bg-white border border-zinc-200 shadow-sm">
                    <h3 className="text-[10px] font-medium  text-zinc-400 uppercase tracking-[0.2em] mb-2">No se encontraron registros</h3>
                        ...
                        <table className="min-w-full leading-normal">
                            <thead>
                                <tr className="bg-zinc-900 border-b border-zinc-800 text-white">
                                    ...
                                    <th className="px-4 py-1.5 text-[12px] font-medium text-white uppercase tracking-widest text-left">Cliente Principal</th>
                                    <th className="px-4 py-1.5 text-[12px] font-medium text-white uppercase tracking-widest text-left">Usuario</th>
                                    <th className="px-4 py-1.5 text-[12px] font-medium text-white uppercase tracking-widest text-left">Transporte</th>
                                    <th className="px-4 py-1.5 text-[12px] font-medium text-white uppercase tracking-widest text-center">Items</th>
                                ...
                                {shipments.map(s => (
                                    <React.Fragment key={s.id}>
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-1.5 text-[11px] font-medium  text-[#285f94]">#{s.id}</td>
                                            ...
                                            <td className="px-4 py-1.5 text-center">
                                                <span className="text-[10px] font-medium  text-zinc-900 bg-zinc-100 px-2 py-0.5 rounded">
                                                    {s.total_orders}
                                                </span>
                                            </td>
                                            <td className="px-4 py-1.5 text-center">
                                                <span className={`px-2 py-0.5 text-[9px] font-medium  uppercase tracking-tight rounded border ${s.status === 'active'
                                                    ...
                                                    {s.status === 'active' && (
                                                        <>
                                                            <Link
                                                                ...
                                                                className="text-[9px] font-medium  uppercase tracking-widest text-zinc-600 hover:text-[#285f94] transition-colors leading-none"
                                                                title="Imprimir"
                                                            >
                                                                Print
                                                            </Link>
                                                            <button
                                                                ...
                                                                className="text-[9px] font-medium  uppercase tracking-widest text-zinc-500 hover:text-red-600 transition-colors leading-none"
                                                                title="Anular"
                                                            >
                                                                Anular
                                                            </button>
                                                ...
                                                <td colSpan="9" className="px-10 py-4 border-b border-zinc-100">
                                                    <div className="bg-white border border-zinc-200 p-6 shadow-sm">
                                                        <div className="flex justify-between items-start mb-6 border-b border-zinc-50 pb-4">
                                                            <div>
                                                                <h4 className="text-[9px] font-medium  text-zinc-400 uppercase tracking-[0.2em] mb-2">Observaciones del Envío</h4>
                                                                <p className="text-[11px] text-zinc-600 italic">
                                                                    {s.note || "SIN OBSERVACIONES REGISTRADAS"}
                                                                </p>
                                                            </div>
                                                            <div className="text-right">
                                                                <span className="text-[8px] font-medium  text-zinc-300 uppercase tracking-widest block">Consolidado por</span>
                                                                <span className="text-[10px] font-medium  text-zinc-900 uppercase">{s.username}</span>
                                                            </div>
                                                        </div>

                                                        <h4 className="text-[9px] font-medium  text-zinc-400 uppercase tracking-[0.2em] mb-4">Pedidos Agrupados</h4>
                                                            ...
                                                            {s.audits.map(a => (
                                                                <div key={a.audit_id} className="bg-zinc-50 border border-zinc-100 p-3 hover:border-zinc-200 transition-all">
                                                                    <div className="flex justify-between items-center mb-2">
                                                                        <span className="text-[11px] font-medium  text-[#285f94]">{a.order_number}</span>
                                                                    ...
                                                                    <div className="text-[8px] font-medium  text-zinc-400 uppercase tracking-tighter">
                            ...
                            <div key={s.id} className={`bg-white border border-zinc-200 p-4 shadow-sm ${s.status === 'cancelled' ? 'opacity-60' : ''}`} onClick={() => setExpandedId(expandedId === s.id ? null : s.id)}>
                                <div className="flex justify-between items-start mb-2">
                                    <div className="flex flex-col">
                                        <span className="text-[12px] font-medium  text-[#285f94] tracking-tight">ENVÍO #{s.id}</span>
                                        <span className="text-[8px] text-zinc-400 uppercase tracking-widest">{formatDate(s.created_at)}</span>
                                    </div>
                                    <span className={`px-2 py-0.5 text-[8px] font-medium  uppercase tracking-tight rounded border ${s.status === 'active'
                                ...
                                <div className="flex justify-between items-center pt-2 border-t border-zinc-50">
                                    <span className="text-[8px] font-medium  text-zinc-400 uppercase tracking-widest">{s.total_orders} PEDIDOS</span>
                                    {s.status === 'active' && (
                                        <div className="flex gap-4" onClick={e => e.stopPropagation()}>
                                            <Link to={`/shipments/print/${s.id}`} className="text-[9px] font-medium  uppercase text-zinc-400 hover:text-zinc-900">Print</Link>
                                            <button onClick={() => handleCancel(s.id)} className="text-[9px] font-medium  uppercase text-zinc-300 hover:text-red-500">Anular</button>
                                        </div>
                                    )}
                                </div>
                                    ...
                                    <div className="mt-4 pt-4 border-t border-zinc-100 space-y-2">
                                        {s.audits.map(a => (
                                            <div key={a.audit_id} className="flex justify-between items-center text-[9px] bg-zinc-50 p-2 rounded">
                                                <div className="flex flex-col">
                                                    <span className="font-medium  text-zinc-800">{a.order_number}</span>
                                                    <span className="text-zinc-400 text-[8px]">{a.despatch_number}</span>
                                                </div>
                                                <div className="text-right">
                                                    <div className="font-medium  text-zinc-600">{a.packages} BULTOS</div>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
