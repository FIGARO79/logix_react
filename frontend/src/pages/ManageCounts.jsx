        ...
        <div className="max-w-[1920px] mx-auto px-4 py-6 font-sans text-sm text-[#32363a]">
                ...
                <div className="bg-[#fffdf5] border border-[#ffecb3] border-l-4 border-l-[#e9730c] rounded-lg py-3 px-6 shadow-sm">
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-x-8 gap-y-4">
                        <div className="flex items-center text-[#e9730c] shrink-0">
                            <h2 className="text-sm font-medium  uppercase tracking-tight">Corrección de Ubicaciones Cerradas</h2>
                        </div>
                        
                        <form onSubmit={handleReopenLocation} className="flex flex-col md:flex-row items-end gap-4 flex-grow lg:justify-end">
                            <div className="w-full md:w-32">
                                <label className="block text-[9px] uppercase tracking-widest font-medium  text-[#6a6d70] mb-1">ID Sesión</label>
                            ...
                            <div className="w-full md:w-48">
                                <label className="block text-[9px] uppercase tracking-widest font-medium  text-[#6a6d70] mb-1">Código Ubicación</label>
                            ...
                            <button type="submit" className="w-full md:w-auto bg-[#e9730c] hover:bg-[#d1670b] text-white font-medium  uppercase tracking-widest px-6 rounded h-9 text-[10px] transition-all shadow-sm">
                                Reabrir
                            </button>
                        </form>
                    </div>
                    {adminMsg && <div className="mt-2 text-green-600 text-[10px] font-medium  animate-pulse">{adminMsg}</div>}
                </div>
            </div>
            ...
            <div className="bg-white shadow-sm rounded border border-[#d9d9d9] overflow-hidden">
                <div className="bg-[#f2f2f2] px-4 py-3 border-b border-[#e5e5e5] flex items-center justify-between">
                    <h2 className="text-[11px] font-medium  uppercase tracking-widest text-[#32363a]">Base de Datos de Auditoría</h2>
                    <span className="text-[10px] px-2 py-1 rounded bg-white border border-gray-200 text-[#6a6d70] font-medium ">{counts.length} REGISTROS TOTALES</span>
                </div>
                        ...
                        <thead className="bg-[#1e4a74] text-white sticky top-0 z-10">
                            <tr>
                                {['ID', 'Sesión', 'Etapa', 'Auditor', 'Fecha / Hora', 'Item Code', 'Descripción', 'Ubicación', 'Cant. Física', 'Acciones'].map((h, i) => (
                                    <th key={i} className="px-4 py-3 text-left text-[10px] font-medium  uppercase tracking-widest whitespace-nowrap">
                                        {h}
                                    </th>
                                ))}
                            </tr>
                            ...
                            {loading ? (
                                ...
                                counts.map((c) => (
                                    <tr key={c.id} className="hover:bg-[#f5f5f5] transition-colors border-b border-gray-50">
                                        ...
                                        <td className="px-4 py-2 text-xs font-medium  text-blue-600">E{c.inventory_stage || '1'}</td>
