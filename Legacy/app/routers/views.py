...
def planner_page(request: Request, username: str = Depends(login_required)):
    """Página del planificador de conteos."""
    if not isinstance(username, str):
        return username
...
@router.get('/manage_count_differences', response_class=HTMLResponse)
async def manage_count_differences(request: Request, username: str = Depends(login_required)):
    """Página para gestionar y editar diferencias en conteos."""
    if not isinstance(username, str):
        return username
    
    return templates.TemplateResponse("manage_count_differences.html", {
        "request": request
    })
...
async def reconciliation_page(request: Request, archive_date: Optional[str] = None, username: str = Depends(login_required), db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        # Obtener ubicación desde el LOG FILTRADO (solo GRNs actuales)
