...
async def reset_user_password(
    ...
    stmt = (
        update(User).where(User.id == user_id).values(password_hash=new_hashed_password)
    )
    result = await db.execute(stmt)
    await db.commit()
    return result.rowcount > 0
...
def permission_required(module: str | List[str]) -> Callable:
    ...
    async def _check_permission(request: Request, db: AsyncSession = Depends(get_db)):
        ...
        if not has_permission:
            detail_msg = (
                f"Acceso denegado: Se requiere permiso '{module}'"
                if isinstance(module, str)
                else f"Acceso denegado: Se requiere uno de los permisos {required_modules}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail=detail_msg
            )
        return username
