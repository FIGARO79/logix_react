    ...
    try:
        ...
        def find_col(targets):
            for t in targets:
                ...
                for c in cols:
                    if c.replace(" ", "").lower() == t_norm:
                        return c
            return None

        mapping = {
            "import_reference": find_col(
                ["IMPORT REFERENCE", "Import_Reference", "import_reference"]
            ),
            "waybill": find_col(["WAYBILL", "Waybill", "waybill"]),
            "grn_number": find_col(
                ["GRN1NUMBER", "GRN1 NUMBER", "grn_number", "GRN_Number"]
            ),
        ...
        df = df.select(
            [pl.col(v).alias(k) for k, v in mapping.items() if v is not None]
        )

        # Transformaciones de datos
        df = df.with_columns(
            [
                pl.col("import_reference")
                .cast(pl.Utf8)
                .str.strip_chars()
                .str.to_uppercase(),
            ...
            (pl.col("import_reference").is_not_null())
            & (pl.col("waybill").is_not_null())
        )
        ...
        print(
            f"[POLARS] Sincronizando {total_items} registros con la Base de Datos...",
            flush=True,
        )
        ...
        for i in range(0, total_items, chunk_size):
            ...
            if is_sqlite:
                ...
                update_dict = {
                    ...
                    if k not in ["import_reference", "waybill"]
                }
