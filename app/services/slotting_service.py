...
class SlottingService:
    ...
    async def _get_item_hits(
        self, db: AsyncSession, item_code: str, days: int = 90
    ) -> int:
        ...
        try:
            ...
            stmt = select(func.count(Log.id)).where(
                and_(Log.itemCode == item_code, Log.timestamp >= since_date)
            )
    ...
    async def _get_layout_config(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        if rules_sql:
            turnover = {
                r.sic_code: {"range": r.description, "spot": r.ideal_spot}
                for r in rules_sql
            }
    ...
    async def get_suggested_bin(
        self, db: AsyncSession, item_details: Dict[str, Any]
    ) -> Optional[str]:
        ...
        heavy_levels = [
            int(lvl.strip())
            for lvl in str(zone_rules.get("heavy_levels", "3, 4, 5")).split(",")
            if lvl.strip().isdigit()
        ]
        high_rotation_levels = [
            int(lvl.strip())
            for lvl in str(zone_rules.get("high_rotation_levels", "0, 1")).split(",")
            if lvl.strip().isdigit()
        ]
        ...
        default_levels = [
            int(lvl.strip())
            for lvl in str(zone_rules.get("default_levels", "2")).split(",")
            if lvl.strip().isdigit()
        ]
        exile_levels = [
            int(lvl.strip())
            for lvl in str(zone_rules.get("exile_rack_levels", "2, 3")).split(",")
            if lvl.strip().isdigit()
        ]
        exile_sics = [
            s.strip().upper()
            for s in str(zone_rules.get("exile_sic_codes", "0, Z, L")).split(",")
            if s.strip()
        ]
        ...
        if is_cantilever:
            ...
            if weight > heavy_weight_min:
                ...
                if not target_levels:
                    target_levels = [
                        3
                    ]  # Fallback seguro de exilio para peso pesado (del 3 en adelante)
            else:
        ...
        for bin_code, info in storage.items():
            ...
            if target_levels and level not in target_levels:
                continue
            if target_score_min is not None and score < target_score_min:
                continue
            if target_score_max is not None and score > target_score_max:
                continue
    ...
    async def get_occupancy_report(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        for bin_code, info in storage.items():
            ...
            if level not in report["zones"][zone]["levels"]:
                report["zones"][zone]["levels"][level] = {
                    "total": 0,
                    "occupied_skus": 0,
                    "full_bins": 0,
            ...
            if current_skus > 0:
                ...
                if aisle != "N/A":
                    aisles_by_items[aisle] = (
                        aisles_by_items.get(aisle, 0) + current_skus
                    )
        ...
        if report["summary"]["total_bins"] > 0:
            report["summary"]["occupancy_pct"] = round(
                (report["summary"]["filled_bins"] / report["summary"]["total_bins"])
                * 100,
                1,
            )
            report["summary"]["avg_items_per_bin"] = (
                round(total_items / report["summary"]["filled_bins"], 1)
                if report["summary"]["filled_bins"] > 0
                else 0
            )

        report["analytics"]["zones_by_items"] = dict(
            sorted(zones_by_items.items(), key=lambda x: x[1], reverse=True)[:5]
        )
        report["analytics"]["top_aisles"] = dict(
            sorted(aisles_by_items.items(), key=lambda x: x[1], reverse=True)[:5]
        )
        report["analytics"]["bins_by_zone"] = {
            z: data["total"] for z, data in report["zones"].items()
        }
        report["analytics"]["bins_by_zone"] = dict(
            sorted(
                report["analytics"]["bins_by_zone"].items(),
                key=lambda x: x[1],
                reverse=True,
            )
        )
