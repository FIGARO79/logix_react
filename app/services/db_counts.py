...
async def get_active_session_for_user(
    ...
    result = await db.execute(
        select(CountSession)
        .where(
            CountSession.user_username == username, CountSession.status == "in_progress"
        )
...
async def close_count_session(
    db: AsyncSession, session_id: int, username: str
) -> Dict[str, str]:
    """Cierra una sesión de conteo."""
    # Verificar que la sesión pertenece al usuario
    result = await db.execute(
        select(CountSession).where(
            CountSession.id == session_id, CountSession.user_username == username
        )
    )
    session = result.scalar_one_or_none()
    ...
    if not session:
        raise HTTPException(
            status_code=403,
            detail="No tienes permiso para cerrar esta sesión o no existe.",
        )
...
async def close_location_in_session(
    db: AsyncSession, session_id: int, location_code: str, username: str
) -> Dict[str, str]:
    ...
    if not session:
        raise HTTPException(
            status_code=403, detail="La sesión no es válida o no te pertenece."
        )

    # Verificar si ya existe el registro de ubicación
    result_loc = await db.execute(
        select(SessionLocation).where(
            SessionLocation.session_id == session_id,
            SessionLocation.location_code == location_code,
        )
    )
    ...
    await db.commit()
    return {
        "message": f"Ubicación {location_code} cerrada para la sesión {session_id}."
    }


async def reopen_location_in_session(
    db: AsyncSession, session_id: int, location_code: str, username: str
) -> Dict[str, str]:
...
async def get_locations_for_session(
    db: AsyncSession, session_id: int, username: str
) -> List[Dict[str, Any]]:
    ...
    if not result.scalar_one_or_none():
        raise HTTPException(
            status_code=403, detail="No tienes permiso para ver esta sesión."
        )

    result_locs = await db.execute(
        select(SessionLocation).where(SessionLocation.session_id == session_id)
    )
    locations = result_locs.scalars().all()

    return [
        {"location_code": loc.location_code, "status": loc.status} for loc in locations
    ]
...
async def get_counts_for_location(
    ...
    if not result.scalar_one_or_none():
        raise HTTPException(
            status_code=403, detail="No tienes permiso para ver estos datos."
        )
    ...
    result_counts = await db.execute(
        ...
        .where(
            ...
            StockCount.counted_location == location_code,
        )
