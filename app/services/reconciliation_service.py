...
async def get_reconciliation_calculations(db: AsyncSession, archive_date: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        final = final.sort(["ir_map", "Item_Code", "grn_map"])
        final = final.with_columns([
            pl.col("grn_map").cum_count().over(["ir_map", "Item_Code"]).alias("_row_num"),
            pl.col("grn_map").count().over(["ir_map", "Item_Code"]).alias("_group_size"),
        ]).with_columns(
