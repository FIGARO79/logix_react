...
async def get_slotting_summary(
    admin: str = Depends(permission_required("inventory")),
    ...
    try:
        ...
        query = text(
            "SELECT bin_1, COUNT(*) as item_count FROM master_items WHERE physical_qty > 0 AND bin_1 IS NOT NULL GROUP BY bin_1"
        )
        ...
        avg_items_per_bin = (
            round(total_items_in_bins / in_use_count, 1) if in_use_count > 0 else 0
        )
        ...
        return {
            ...
            "occupancy_pct": round((in_use_count / total_locations * 100), 1)
            if total_locations > 0
            else 0,
            "by_zone": zones,
            ...
            "zones_by_items": dict(
                sorted(zone_items.items(), key=lambda x: x[1], reverse=True)
            ),
            "top_aisles": dict(
                sorted(aisle_items.items(), key=lambda x: x[1], reverse=True)[:5]
            ),
        }
...
async def update_slotting_config(
    data: dict = Body(...),
    admin: str = Depends(permission_required("inventory")),
    db: AsyncSession = Depends(get_db),
):
...
async def get_slotting_template(admin: str = Depends(permission_required("inventory"))):
    ...
    return Response(
        ...
        headers={"Content-Disposition": "attachment; filename=layout_almacen.xlsx"},
    )


@router.post("/slotting-upload")
async def upload_slotting_config(
    ...
    try:
        ...
        for r in df.iter_rows(named=True):
            ...
            try:
                ...
                nivel = (
                    int(float(str(raw_nivel)))
                    if raw_nivel is not None and str(raw_nivel).lower() != "nan"
                    else 0
                )
            except:
                nivel = 0
            ...
            try:
                ...
                score = (
                    int(float(str(raw_score)))
                    if raw_score is not None and str(raw_score).lower() != "nan"
                    else 0
                )
            except:
                score = 0
            ...
            db.add(
                BinLocation(
                    ...
                    level=str(
                        info["level"]
                    ),  # El modelo espera String para level en este proyecto
                    spot=info["spot"],
                    score=info["score"],
                )
            )
...
async def get_admin_users_api(
    db: AsyncSession = Depends(get_db), admin: bool = Depends(admin_login_required)
):
...
async def approve_user_api(
    ...
    if success:
        return ORJSONResponse(
            content={"success": True, "message": f"Usuario {user_id} aprobado"}
        )
...
async def delete_user_api(
    ...
    if success:
        return ORJSONResponse(
            content={"success": True, "message": f"Usuario {user_id} eliminado"}
        )
    raise HTTPException(status_code=404, detail="Usuario no encontrado")


@router.post("/reset_password/{user_id}")
async def admin_reset_password_api(
    user_id: int,
    ...
    if success:
        return ORJSONResponse(
            content={
                "success": True,
                "message": f"Contraseña restablecida para usuario {user_id}",
            }
        )
    raise HTTPException(
        status_code=400,
        detail="Error al restablecer contraseña o contraseña no cumple requisitos",
    )
...
async def update_user_permissions_api(
    ...
    if result.rowcount > 0:
        return ORJSONResponse(
            content={"success": True, "message": "Permisos actualizados"}
        )
    raise HTTPException(status_code=404, detail="Usuario no encontrado")
