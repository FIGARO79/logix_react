...
async def debug_auth(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return {
        ...
        "permissions_list": user.permissions.split(",")
        if user and user.permissions
        else [],
        "session_keys": list(request.session.keys()),
        "has_inbound_permission": "inbound"
        in (user.permissions.split(",") if user and user.permissions else []),
    }
...
async def delete_grn_bulk(
    ...
    if payload.password != ADMIN_PASSWORD:
        raise HTTPException(
            status_code=401, detail="Contraseña de administrador incorrecta"
        )
    ...
    for grn in grns_to_delete:
        ...
        stmt_recon = delete(ReconciliationHistory).where(
            ReconciliationHistory.grn == grn
        )
    ...
    if os.path.exists(GRN_CSV_FILE_PATH):
        try:
            ...
            df_280 = df_280.filter(
                ...
                .str.strip_chars()
                .str.to_uppercase()
                .is_in(grns_to_delete)
            )

            if df_280.height < initial_count:
                df_280.write_csv(GRN_CSV_FILE_PATH)
                print(
                    f"CSV 280 actualizado: {initial_count - df_280.height} filas eliminadas."
                )
        except Exception as e:
    ...
    return {
        "message": f"Se han eliminado {len(grns_to_delete)} GRNs del archivo 280 y se han limpiado los registros vinculados en todo el sistema."
    }
