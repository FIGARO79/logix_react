...
def get_homography_matrix(qr_pts, real_size):
    ...
    dst_pts = np.array(
        [[0, 0], [real_size, 0], [real_size, real_size], [0, real_size]],
        dtype="float32",
    )
...
async def measure_box_v2(request: MeasureRequest):
    try:
        ...
        if img is None:
            return MeasureResponse(error="Imagen inválida")
        ...
        if qr_pts is None:
            ...
            return MeasureResponse(
                error="El servidor no pudo validar el QR en la foto. Evite reflejos sobre el papel."
            )

        # 2. Calcular Matriz de Homografía (Mapeo Píxel -> CM en el suelo)
        H = get_homography_matrix(qr_pts, request.qr_real_size)
        if H is None:
            return MeasureResponse(error="Error en calibración 3D.")
        ...
        if request.gizmo:
            ...
            p_test_cm = np.array(
                [[[p_origin_cm[0], p_origin_cm[1] + 1]]], dtype="float32"
            )
            ...
            logger.info(
                f"Medición: L={length_cm:.1f}, W={width_cm:.1f}, H={height_cm:.1f} (Escala: {local_px_per_cm:.2f}, Factor: {correction_factor})"
            )
